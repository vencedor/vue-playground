                  ...
                  <draggable :list="eachFormObj.fields" class="dragArea" :group="{ name:'formbuilder', pull:false, put:true }" :sort="true" ghost-class="sortable__ghost">
                    ...
                    <el-col v-for="(field, index) in eachFormObj.fields" :key="index" v-bind="field" :span="field.span" class="form__group" :class="{ 'is--active': field === activeField }">
                      ...
                      <div class="form__actiongroup">
                        ...
                        <el-button-group class="form__actionlist_right">
                          <el-button circle size="mini" icon="el-icon-arrow-left" @click="resizeElement(field,-1)"></el-button>
                          <el-button circle size="mini" icon="el-icon-arrow-right" @click="resizeElement(field,1)"></el-button>
                        </el-button-group>
...
export default {
  ...
  created: function(){
    document.getElementById('loadJSONdataclick').addEventListener('click',this.loadJSON);
  },
  ...
  methods: {
    ...
    resizeElement(field, incDec=1) {
      FormBuilder.resizeElement(field, incDec)
    },
    loadJSON(){
      console.log(document.getElementById('jsonstore').value.toString());
      if(document.getElementById('jsonstore').value.toString()!=''){
        console.log(document.getElementById('jsonstore').value);
        this.forms=JSON.parse(document.getElementById('jsonstore').value);
        console.log(this.forms);
      }
    },
    editElementProperties(field) {

      document.getElementById('jsonstore').value=JSON.stringify(this.forms);
      console.log(document.getElementById('jsonstore').value);
      console.log(JSON.stringify(this.forms));

      console.log("editElementProperties:: form ->", this.forms);
      console.log("editElementProperties:: activeField ->", this.activeField);
...
.empty-section {
  text-align: center;
  font-size: 30px;
  background: linear-gradient(to bottom,#4786f2,#3a6cd7);
...
.form__group {
    ...
    &:hover {
        ...
        .form__actionitem--move,.form__actionlist_right {
            visibility: visible;
        }
    }
